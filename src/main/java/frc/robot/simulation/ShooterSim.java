...
public class ShooterSim {
  ...
  private final SingleJointedArmSim m_joint = new SingleJointedArmSim(
      k_pivotMotor,
      k_pivotGearRatio,
      SingleJointedArmSim.estimateMOI(Units.inchesToMeters(RobotConstants.config.Shooter.k_length),
          RobotConstants.config.Shooter.k_mass),
      RobotConstants.config.Shooter.k_length,
      RobotConstants.config.Shooter.k_minAngle,
      RobotConstants.config.Shooter.k_maxAngle,
      true,
      RobotConstants.config.Shooter.k_startingAngle);

  private Mechanism2d m_mech2d = null;

  private final Translation2d m_origin = new Translation2d(
      (RobotConstants.config.Simulation.k_width / 2) + RobotConstants.config.Shooter.k_distanceFromCenter, 0);
  ...
  private ShooterSim(Mechanism2d mech2d) {
    ...
    m_shooterPivot = m_shooterBase.append(
        new MechanismLigament2d(
            "ShooterPivot",
            RobotConstants.config.Shooter.k_pivotHeight,
            90,
            4,
